...
describe('resolveFile()', () => {
  it('works', async () => {
    ...
    await expect(resolveFile({ path: () => '/etc/passwd' } as uri.URI)).resolves.toEqual('root:*');
  });

  it('handles failures', async () => {
    ((readFile as unknown) as jest.Mock).mockImplementation((_, __, cb) => cb(new Error('Using shadow, ha, ha')));
    await expect(resolveFile({ path: () => '/etc/passwd' } as uri.URI)).rejects.toThrowError('Using shadow, ha, ha');
  });
});
